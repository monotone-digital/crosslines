# The Crosslines — Site Content

Source inventory of all marketing and product copy currently live across the repo. Structured content modules live in `content/`; page-level copy lives in `app/**/page.tsx` and shared components.

**Site:** https://thecrosslinesgroup.com  
**Brand:** The Crosslines  
**Tagline / positioning:** Made in Multan. Installed across Punjab. Supported for life.

---

## Company facts

| Claim | Copy |
| --- | --- |
| Experience | 6 years manufacturing and installing |
| Volume | 2,000+ projects completed |
| Profile warranty | 10 years on the profile |
| Hardware warranty | 12 months on hardware |
| Support | Lifetime customer support |
| Manufacturing | Multan |
| Installation (Punjab) | Manufacture, deliver and install |
| Supply (rest of Pakistan) | Manufacture and deliver; installation arranged at client end |
| Owner / contact person | Umer |

---

## Contact & social

| Channel | Value |
| --- | --- |
| Phone / WhatsApp | +92 345 7291119 |
| WhatsApp deep link | https://wa.me/923457291119 |
| Email | thecrosslinesgroup@gmail.com |
| Instagram | https://www.instagram.com/thecrosslinesfactory/ (`@thecrosslinesfactory`) |
| TikTok | https://www.tiktok.com/@the.crosslines |
| Address | Head Office, Gujjar Chowk, Qadirpur Ran Bypass, Multan |
| Maps | https://share.google/h7Ayskc5JrnsqtNBu |
| Business hours | Confirm before visiting |

### Default WhatsApp messages

- General: `Hello The Crosslines, I would like to discuss a project.`
- To Umer: `Hello Umer, I would like to discuss a Crosslines project.` / `Hello Umer, I would like to discuss a project.`
- Conservatory survey: `Hello Umer, I would like to book a conservatory site survey.`

---

## Site map & navigation

**Primary nav:** Windows · Doors · Conservatories · Profiles · Projects · About · Contact

| Route | Page number | Meta title | Meta description |
| --- | --- | --- | --- |
| `/` | — | The Crosslines \| uPVC Windows & Doors in Multan | uPVC windows, doors and conservatories manufactured in Multan and installed across Punjab, with a ten-year profile warranty and lifetime support. |
| `/windows` | 02 / 08 | uPVC Windows in Multan | Made-to-measure casement, sliding and fixed uPVC windows for homes and building projects across Punjab. |
| `/doors` | 03 / 08 | uPVC Doors in Multan | Made-to-measure casement, sliding and lift-and-slide uPVC doors manufactured in Multan and installed across Punjab. |
| `/conservatories` | 04 / 08 | uPVC Conservatories in Multan | Light-filled uPVC conservatories designed around orientation, shade and year-round use in South Punjab. |
| `/systems` | 05 / 08 | uPVC Profiles & Systems in Multan | Compare five uPVC profile manufacturers across three tiers and learn what to look for in profile construction, seals, reinforcement and colour performance. |
| `/projects` | 06 / 08 | Projects | A growing archive of residential, commercial and builder uPVC projects by The Crosslines across Punjab. |
| `/about` | 07 / 08 | About | Meet the Multan manufacturer behind more than 2,000 uPVC window, door and conservatory projects. |
| `/contact` | 08 / 08 | Contact & Request a Quote | Request a quote for uPVC windows, doors or conservatories from The Crosslines in Multan. |

**OG / Twitter:** Made in Multan. Installed across Punjab. Supported for life.  
**Image:** `/og.webp`

---

## Shared UI copy

### Warranty strip

> Ten years on the profile. Lifetime support on every window we fit.  
> 12 months on hardware

### Default CTA band

- **Title:** Tell us about the opening. We will tell you what fits.
- **Copy:** Send the details to Umer on WhatsApp and we will come back with a specification and a price.
- **CTAs:** WhatsApp Umer · Request a quote

### Profile tier strip

- **Eyebrow:** Profiles we offer
- **Copy:** Five manufacturers across three clearly numbered tiers.
- **CTA:** Compare profiles → `/systems`

### Footer

> uPVC windows, doors and conservatories—manufactured in Multan and installed across Punjab.

---

## Homepage `/`

### Hero

- **Eyebrow:** Made in Multan · Installed across Punjab
- **Italic lead:** Windows that belong to the building.
- **H1:** We make them. We fit them. / We stay afterwards.
- **Supporting:** uPVC windows, doors and conservatories, manufactured and installed by one accountable team.
- **CTAs:** Start on WhatsApp · Request a quote
- **Footer strip:** uPVC · GLASS · INSTALLATION
- **Image caption:** Complete responsibility — Frame, glass, hardware and fitting—from one manufacturer.

### Proof strip

| Value | Label |
| --- | --- |
| 6 years | Manufacturing and installing |
| 2,000+ | Projects completed |
| 10 years | Warranty on the profile |
| Lifetime | Customer support |

### What we make

- **Eyebrow:** What we make
- **Aside:** Designed opening by opening, then made and fitted by our own team.
- **H2:** The best window is not a product off a shelf. It is the right system for the room around it.

| # | Title | Copy | Href |
| --- | --- | --- | --- |
| 01 | Windows | Casement, sliding and fixed systems, made to the exact size of every opening. | `/windows` |
| 02 | Doors | Casement, sliding and lift & slide doors engineered around their span and weight. | `/doors` |
| 03 | Conservatories | Light-filled garden rooms designed around orientation, shade and a Multan summer. | `/conservatories` |
| 04 | Glass | Single, double and triple glazing specified room by room—not from a price list. | `/windows#glass` |

### Three tiers of profile

- **Eyebrow:** Three tiers of profile
- **H2:** We will tell you which one you need.
- **Copy:** We carry several systems across three tiers, which means we have no reason to push you past what your project requires.
- **CTA:** Compare the systems
- **Image caption:** Three profile tiers. One honest recommendation.

*(Tier titles/summaries/brands — see Profile tiers below.)*

### Built for South Punjab

- **Eyebrow:** Built for South Punjab
- **H2:** Heat, dust and sun decide how a window performs here.
- **Badge:** South Punjab · Summer conditions

| Topic | Copy |
| --- | --- |
| Heat | The frame’s thermal figure tells you how much it works against your air conditioning through summer. |
| Dust | Two seals is standard. Three holds a closed window tighter against the dust South Punjab lives with. |
| Sun | Profile grade and stabiliser chemistry decide whether a frame keeps its colour and shape. |

### One accountable team

- **Eyebrow:** One accountable team
- **H2:** Fitted by us. Guaranteed by us.
- **Copy:** Across Punjab, we manufacture, deliver and install. One company is responsible for the frame, glass, hardware and fitting—and one company answers afterwards.
- **Aside:** Across the rest of Pakistan, we manufacture and deliver. Installation is arranged at your end.

### Recent work (placeholders)

- **Eyebrow:** Recent work
- **H2:** Projects in preparation.
- **CTA:** See the project archive
- **Cards:** Project 01–03 — Photography and details awaiting client approval.

### Testimonials (placeholders)

- **Eyebrow:** What clients say
- **Copy:** Verified client stories will appear here after names, cities and product details are approved.
- **Cards:** Testimonial 01–03 — Awaiting client approval

---

## Windows `/windows`

### Hero

- **Title:** Made to measure. Never standard size.
- **Summary:** Every type is available across all three profile tiers. The type decides how it opens. The tier decides how it ages.

### Four window types

- **Eyebrow:** Four window types
- **Aside:** One opening often uses more than one type. We work it out room by room.
- **H2:** Start with how the room needs to work.

| # | Type | Movement | Opening | Best for | Copy | Limited by |
| --- | --- | --- | --- | --- | --- | --- |
| 01 | Openable / casement | Side-hinged | Full sash | Bedrooms, living rooms and exposed elevations | The default and the best-sealing window we make. The sash presses into the frame so the seal compresses rather than slides. | It needs clear space to swing. On a narrow side passage or above a footpath, a slider may be the better answer. |
| 02 | Sliding, 2-track | Slides horizontally | Up to half | Compact rooms, passages and everyday openings | Two sashes on two tracks, each passing the other. Nothing swings into the room and half the opening can stand open. | A sliding sash brushes past its track, so it will never seal as tightly as a casement on an exposed or dusty elevation. |
| 03 | Sliding, 3-track | Three sliding planes | Up to two-thirds | Wide openings or a dedicated insect-mesh track | A wider system for more glass in motion, or for carrying an insect mesh on a track of its own. | The frame is deeper and there is more track to keep clean. |
| 04 | Fixed | No moving parts | None | Views, daylight and hard-to-reach glazing | Light and view with nothing to open or seal against. The tightest and most economical option per square foot. | No ventilation, so it is usually paired with an openable sash in the same run. |

### Glass (`#glass`)

- **Eyebrow:** Glass
- **H2:** One pane, two or three.
- **Copy:** The sealed cavity between panes slows heat because trapped air conducts it poorly. In Multan, that earns its money on a summer afternoon—and again on a main road, where the second pane cuts noise.
- **Thicknesses:** 10mm — Our standard · 8mm — For smaller openings · 12mm — For large spans or noise
- **Aside:** A bedroom on a main road and a store room off an internal courtyard are not the same problem. We specify the glass per opening.

### Finishes

- **Eyebrow:** Finishes
- **H2:** White, woodgrain and black.

| Finish | Description |
| --- | --- |
| White | Clean and architectural |
| Woodgrain laminate | Warm, natural character |
| Black | Crisp, contemporary contrast |

> You choose from physical swatches, so the colour you pick is not dependent on lighting in somebody else’s photograph.

### The frame decides the rest

- **Eyebrow:** The frame decides the rest
- **H2:** The part you cannot change later.
- **Copy:** Glass, hardware and seals all fit into the profile. Its grade, chambers, reinforcement and seals decide how the window performs long after fitting.
- **Aside:** Most of that does not show in a showroom. It shows in year eight, in a frame that still closes tightly against dust and a colour that has not moved.
- **CTA:** See the three tiers

---

## Doors `/doors`

### Hero

- **Title:** Built for the weight they carry.
- **Summary:** A door is heavy, moves many times a day and carries a lock. We specify the frame, hardware and glass around those three facts.

### Door types

- **Eyebrow:** Door types
- **H2:** The right movement for the span.

| # | Type | Movement | Copy | Consider |
| --- | --- | --- | --- | --- |
| 01 | Casement doors | Hinged movement | Single or double-leaf hinged doors with the tightest compression seal we offer. | They require swing space, and wind can catch an open leaf. |
| 02 | Sliding doors | Two or three tracks | Two- and three-track systems for patios and terraces, with no floor space lost. | The seal is lighter than a casement and wide sashes become heavier to move. |
| 03 | Lift & slide | Lift, roll, then seal | A large sash lifts clear of its seals before rolling, then drops back to compress them when closed. | Its deeper frame, hardware and structural needs should be planned before the opening is built. |

### Lift & slide feature

- **Eyebrow:** Lift & slide
- **H2:** Wide glass. A proper seal.
- **Copy:** Turn the handle and the sash lifts clear of its seal, rolling almost weightlessly. Turn it back and the sash drops to compress the seal like a hinged door.
- **Aside:** Usually the right answer for a large living space opening onto a lawn or terrace. It is worth discussing before the walls go up.
- **CTA:** Discuss the opening

### Hardware matters

- **Eyebrow:** Hardware matters
- **H2:** Doors wear at the parts that move.
- **Copy:** Frames rarely fail. Hinges drop, rollers flatten, handles work loose and multipoint locks stop lining up. That is why hardware must be specified to the weight of the leaf.
- **Parts listed:** Handles · Hinges · Rollers · Multipoint locks
- **Aside:** Hardware is covered for twelve months. After that, we still source and fit replacement parts for anything we installed.

### Glass for doors

- **Eyebrow:** Glass for doors
- **H2:** The largest pane in the room deserves more thought.
- **Copy:** Single, double and triple glazing is available across all door types. We use 10mm as standard, with 8mm and 12mm available where the opening, exposure or noise level calls for it.
- **Aside:** On a door, the case for heavier glazing is stronger: it is usually the pane people stand closest to and the largest area of glass facing outside.

### Page CTA

- **Title:** Bring us the span before the walls go up.
- **Copy:** For wide sliding and lift-and-slide openings, an early conversation makes the finished door better—and prevents expensive corrections later.

---

## Conservatories `/conservatories`

### Hero

- **Title:** A room that belongs to the garden.
- **Summary:** Built in uPVC and designed around how the room will actually be used through a Multan summer.
- **Primary CTA label:** Book a site survey

### Why build one

- **Eyebrow:** Why build one
- **H2:** The walls stop behaving like walls.
- **Copy A:** A conservatory changes how a house gets used: breakfast where the morning light reaches, a room for plants, somewhere to sit in December when the lawn looks its best.
- **Copy B:** It is made mostly of glass, and that is the point. You get the garden without being outside, and the light without the dust.

### Designed from the site

- **Eyebrow:** Designed from the site
- **H2:** Orientation decides almost everything.

| # | Topic | Copy |
| --- | --- | --- |
| 01 | Direction | Where the room faces changes the light it receives and the heat it holds. |
| 02 | Shade | Roof, overhang and surrounding walls determine whether the room remains usable in June. |
| 03 | Glass | The glazing is settled at survey because orientation and use change the answer. |
| 04 | Span | The frame stays as slim as the opening safely allows, preserving the reason for building it. |

### The first step

- **Eyebrow:** The first step
- **H2:** Every conservatory begins with a site visit.
- **Copy:** Partly for measurements. Mostly because the way the space faces decides whether it becomes a room you live in or one you avoid for half the year.
- **CTA:** Book a survey on WhatsApp

### Page CTA

- **Title:** A conservatory cannot be priced well from a list.
- **Copy:** Start with a site survey. We will look at the opening, direction, shade and intended use before specifying the frame and glass.

---

## Profiles & Systems `/systems`

### Hero

- **Title:** Know the profile. Know who stands behind it.
- **Summary:** Five profile manufacturers across three tiers, supported by a practical guide to what matters inside the frame.

### Buyer’s guide

- **Eyebrow:** Buyer’s guide
- **Aside:** None of this is brand language. These are the things worth comparing in writing.
- **H2:** What separates a good window from a cheap one.

| # | Topic | Copy |
| --- | --- | --- |
| 01 | Profile class | EN 12608 grades profile wall thickness. Class A is thicker; Class B is lighter and less expensive to produce. |
| 02 | Chambers | Hollow compartments trap air and slow heat and sound. Most quality systems run five or more. |
| 03 | Seals | Two seals is standard. Three holds a closed sash measurably tighter against South Punjab dust. |
| 04 | Frame heat transfer | Published as Uf in W/m²K. Lower is better. Do not confuse it with the whole-window Uw figure. |
| 05 | Reinforcement | Large windows and doors need steel inside the profile to remain straight. It is invisible once assembled. |
| 06 | UV and colour | Stabiliser chemistry and profile grade determine how the frame behaves under years of intense sun. |

### Six questions worth asking

- **Eyebrow:** Take this to any supplier
- **H2:** Six questions worth asking.

1. Which profile class is it, A or B?
2. How many chambers, and how many seals?
3. What is the frame’s Uf figure?
4. Is the frame reinforced, and where?
5. Is there a written colour guarantee, and how long?
6. Who covers the installation if something fails?

> We are happy to answer all six on anything we quote. Not every supplier will.

### Profile manufacturers intro

- **Eyebrow:** Profile manufacturers
- **Aside:** The company behind a system matters: documentation, continuity and access to the right profile all begin there.
- **H2:** Five companies. Three tiers.
- **Copy:** We select the exact system after understanding the opening, exposure and budget. The companies below show the profile range available through Crosslines.

### How to read the tiers

- **Eyebrow:** How to read the tiers
- **Label:** Crosslines selection guide
- The tiers organise our profile range by origin, documentation, system breadth and local availability. They are not a universal performance score: the right answer still depends on the exact system and opening.
- Exact series names are intentionally excluded here. Any numeric performance figure belongs to the specific system named in your written quote and will not be attributed to a manufacturer as a whole.

### Page CTA

- **Title:** Not sure which tier you need?
- **Copy:** Tell us the building, how long you plan to keep it and how exposed the elevation is. That is usually enough to point you in the right direction.

---

## Profile tiers (`content/profile-brands.ts`)

### Tier 1 — Imported German & Turkish

**Summary:** International manufacturers with broad system portfolios and an established third-party certification footprint.

#### Aluplast (Germany)

- **Summary:** A German uPVC systems manufacturer with an established European certification footprint.
- **Detail:** Aluplast is a German manufacturer of uPVC window, door and sliding systems. Its published certification footprint includes RAL, ift Rosenheim, the Passive House Institute, TÜV Süd ISO 9001, CE and Rewindo.
- **Facts:** Germany · European certification · Window, door & sliding systems

#### ASAŞ / Asaspen (Türkiye)

- **Summary:** The window-systems division of ASAŞ, a Turkish industrial group founded in 1990.
- **Detail:** Founded in 1990, ASAŞ operates across 90 countries and six continents, with a 400,000 m² production facility and approximately 3,000 employees. Asaspen is its window-systems arm, and those systems carry the VERATEC brand.
- **Facts:** Founded 1990 · 90 countries · ASAŞ · Asaspen · VERATEC

### Tier 2 — Imported Turkish

**Summary:** Established Turkish manufacturers with published technical documentation and strong residential value.

#### Wintech (Türkiye)

- **Summary:** An ADO Group profile brand founded in Antalya in 1997, with an established Pakistan channel.
- **Detail:** Wintech is a profile brand of ADO Group, founded in Antalya in 1997. The manufacturer publishes European-standard technical documentation and maintains a dedicated Pakistan contact channel.
- **Facts:** ADO Group · Founded 1997 · Pakistan market channel

#### Winer (Türkiye)

- **Summary:** An Eryap Group profile brand produced in Gaziantep with published Turkish and European certifications.
- **Detail:** Winer is an Eryap Group brand produced at its Gaziantep facilities. Its published certifications include ISO 9001, ISO 14001, TS 5358 EN 12608, CE, TSE and SKZ certification for laminate foils.
- **Facts:** Eryap Group · Gaziantep · ISO · CE · TSE · SKZ

### Tier 3 — Pakistani profiles

**Summary:** Local supply keeps replacement parts accessible and gives rentals and cost-conscious builds a practical option.

#### Skypen (Pakistan)

- **Summary:** A Karachi-based Pakistani profile supplier with sales coverage across major cities.
- **Detail:** Skypen is headquartered in Karachi and publishes sales coverage across Karachi, Lahore and Islamabad. Its public range covers casement, sliding and lift-and-slide categories; comparable profile-performance figures are not currently published.
- **Facts:** Pakistan · Karachi head office · Locally accessible supply

---

## Projects `/projects`

### Hero

- **Title:** Two thousand projects. The archive is next.
- **Summary:** Homes, commercial buildings and builder contracts across Punjab—and supply projects across Pakistan.

### Selected work

- **Eyebrow:** Selected work
- **H2:** Real projects are being documented.
- **Aside:** Each published project will include its city, product, profile tier and year. No generic imagery will be presented as completed work.
- **Filters (UI only):** All products · Windows · Doors · Conservatories · City filter pending
- **Placeholders:** 9 unpublished project slots (`content/projects.ts`) — “Awaiting photography” / “Details to be approved”

### For builders & contractors

- **Eyebrow:** For builders & contractors
- **H2:** Three quotes from one drawing set.
- **Copy:** We can quote all three profile tiers against the same drawings, so you can price the project properly before committing to a specification.
- **Aside:** We manufacture in Multan and install across Punjab. Profiles carry a ten-year warranty, hardware carries twelve months and replacement parts remain available afterwards.
- **CTA:** Request a builder quote

---

## About `/about`

### Hero

- **Title:** We make them—and fit them ourselves.
- **Summary:** One material, one manufacturing team and one point of accountability from quotation to support.

### The Crosslines

- **Eyebrow:** The Crosslines
- **H2:** A deliberate focus on uPVC.
- The Crosslines has been manufacturing in Multan for six years. In that time we have completed more than 2,000 projects for homeowners, builders and architects.
- Working in one material means we know the profiles we carry, can match the tier to the building and are willing to say when the less expensive option is the right one.
- Across Punjab, we install what we manufacture. One company is accountable for the frame, glass, hardware and fitting.
- If something we installed needs attention years later, we remain the people who come and look at it.

### Meet Umer

- **Eyebrow:** The person who answers
- **H2:** Meet Umer.
- **Copy:** Enquiries are answered by Umer, who owns the business. Not a sales team or call centre—the person responsible for the quote, specification and support.
- **CTA:** Message Umer
- **Portrait:** Placeholder (“Portrait to be added”)

### Where we work

- **Eyebrow:** Where we work
- **H2:** Local installation. National supply.

| Region | Copy |
| --- | --- |
| Punjab | We manufacture, deliver and install. Ten-year profile warranty, twelve months on hardware and lifetime support. |
| Rest of Pakistan | We manufacture and deliver. The profile warranty, hardware cover and customer support remain in place. |

> Ten years on profiles · Twelve months on hardware · Lifetime customer support

---

## Contact `/contact`

### Intro

- **Eyebrow:** Contact / 08
- **H1:** Tell us what you need.
- **Copy:** The more you can tell us about the openings, the more useful our first reply will be. If you have drawings, send them separately on WhatsApp.

### Request a quote

- **Eyebrow:** Request a quote
- **H2:** A useful first conversation.
- **Copy:** For conservatories, we will arrange a site survey before quoting. They cannot be priced accurately without one.

### Quote form fields

| Field | Notes |
| --- | --- |
| Name | required |
| Phone / WhatsApp | required |
| City | required |
| Product type | Windows · Doors · Conservatory · Not sure yet |
| Project type | My home · Commercial · I am a builder |
| Approximate openings | optional numeric |
| Anything else we should know? | free text |
| Success | Thank you. Umer will reply on WhatsApp as soon as possible. |
| Privacy note | Your details are sent privately to thecrosslinesgroup@gmail.com. |

### Visit or call

- **Eyebrow:** Visit or call
- **H2:** Based in Multan. Working across Punjab.

*(Contact details — see Contact & social above.)*

---

## Content modules & placeholders

| Module | Path | Status |
| --- | --- | --- |
| Profile brands / tiers | `content/profile-brands.ts` | Live — 5 brands across 3 tiers |
| Projects archive | `content/projects.ts` | 9 unpublished placeholders; awaiting approved photography & details |
| Homepage projects | `app/page.tsx` | 3 placeholder cards |
| Homepage testimonials | `app/page.tsx` | 3 placeholder cards |
| Umer portrait | `app/about/page.tsx` | Placeholder |

---

## Brand assets referenced

| Asset | Path |
| --- | --- |
| Wordmark | `/brand/crosslines-wordmark.png` |
| Mark | `/brand/crosslines-mark.png` |
| Profile logos | `/brand/profiles/{aluplast,asas,wintech,winer,skypen}.*` |
| OG image | `/og.webp` |
